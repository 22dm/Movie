...
public class StatisticsServiceImpl implements StatisticsService {
    ...
    public ResponseVO getMoviePlacingRateByDate() {
        try {
            ...
            Date today = simpleDateFormat.parse(simpleDateFormat.format(new Date()));
            Date startDate = getNumDayAfterDate(today, -6);
            List<MoviePlacingRateByDateVO> moviePlacingVOList = new ArrayList<>();
            for(int i = 0; i < 7; i++){
                MoviePlacingRateByDateVO moviePlacingVO = new MoviePlacingRateByDateVO();
                Date date = getNumDayAfterDate(startDate, i);
                moviePlacingVO.setDate(date);
                List<MoviePlacingRateByDate> moviePlacingList = statisticsMapper.selectMoviePlacingRateByDate(date, getNumDayAfterDate(date, 1));
                double placing;
                if(moviePlacingList.size() == 0 || moviePlacingList.get(0) == null)
                    placing = 0;
                else
                    placing=moviePlacingList.get(0).getPlacing();
                moviePlacingVO.setPlacing(placing);
    ...
    public ResponseVO getPopularMovies(int days, int movieNum) {
        try {
            return ResponseVO.buildSuccess(movieTotalBoxOfficeList2MovieTotalBoxOfficeVOList(statisticsMapper.selectMovieTotalBoxOfficeInDays(days, movieNum)));
        }catch (Exception e){
            e.printStackTrace();
            return ResponseVO.buildFailure("失败");
        }
    ...
    private List<MoviePlacingRateByDateVO> moviePlacingList2moviePlacingVOList(List<MoviePlacingRateByDate> moviePlacingList){
        List<MoviePlacingRateByDateVO> moviePlacingVOList = new ArrayList<>();
        for(MoviePlacingRateByDate moviePlacing : moviePlacingList){
            moviePlacingVOList.add(new MoviePlacingRateByDateVO(moviePlacing));
        }
        return moviePlacingVOList;
    }
